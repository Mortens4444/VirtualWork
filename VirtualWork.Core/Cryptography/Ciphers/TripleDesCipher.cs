...
{
	public static class TripleDesCipher
	{
		public const string KeySizeMustBe16Or24Bytes = "Password must be 16 or 24 characters length.";

		public const string InitializationVectorSizeMustBe8Or16Bytes = "IV must be 8 or 16 characters length.";
			...
			byte[] initializationVector64Bit)
		{
			if (key192Bit == null)
			{
				throw new ArgumentNullException(nameof(key192Bit));
			}
			...
			byte[] initializationVector64Bit)
		{
			...
			if (key192Bit == null)
			{
				throw new ArgumentNullException("key_192_bit");
			}
			if (initializationVector64Bit == null)
			{
				throw new ArgumentNullException("initialization_vector_64_bit");
			}
			if ((key192Bit.Length != 16) && (key192Bit.Length != 24))
			{
				throw new ArgumentException(KeySizeMustBe16Or24Bytes, nameof(key192Bit));
